            ...
            for (int z = -1; z <= 1; z++) {
                ...
                for (int y = -1; y <= 1; y++) {
                    if (y + coordinate < neighborChunk.getMinSection() || y + coordinate >= neighborChunk.getMaxSection())
                        continue;
                    neighborChunk.getSection(y + coordinate).invalidate();
                }
            }
        }
    }
    ...
    private static Set<BlockVec> getNearbyRequired(LightGenerationData data, BlockVec point, LightType type) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                ...
                try {
                    highestRegionPoint = Math.max(highestRegionPoint, chunkCheck.getOcclusionData().highestBlock);
                } finally {
                    chunkCheck.unlockReadLock();
                }
            }
        }
